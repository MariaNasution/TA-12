...
export default function DoctorDashboard() {
    const { address, role, userName, loading } = useAuth();
...
const handleRequestAccess = async (e) => {
    ...
    const handleSaveMedicalData = async (e) => {
        ...
        try {
            ...
            await tx.wait();
            const ringkasanDiagnosa = medicalData.length > 30 
                ? medicalData.substring(0, 30) + "..." 
                : medicalData;
            try {
                await fetch('http://127.0.0.1:5000/notifications/add', {
                    ...
                    body: JSON.stringify({
                        address: patientAddr,
                        pesan: isEditMode 
                        ? `dr. ${userName || 'Dokter'} memperbarui diagnosa: "${ringkasanDiagnosa}"` 
                        : `dr. ${userName || 'Dokter'} menambahkan diagnosa baru: "${ringkasanDiagnosa}"`
                    })
                });
                console.log("Notifikasi berhasil dikirim ke SQLyog");
            } catch (notifErr) {
